...
const generateMockData = (): BatchStone[] => {
  return Array.from({length: 15}).map((_, i) => ({
    id: `ru-bkk3-${i}`,
    code: `RB3-${String(i+1).padStart(3, '0')}`,
    weight: Number((1.2 + Math.random()).toFixed(2)),
    variety: 'Ruby',
        ...
        <div className="p-6 border-b border-stone-100 flex justify-between items-start bg-white sticky top-0 z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-mono text-stone-400 bg-stone-100 px-1.5 py-0.5 rounded">RUBY BATCH (APR 2025)</span>
            </div>
...
export const RuBkk160425Template: React.FC<Props> = ({ tabId, isReadOnly }) => {
  ...
  const filteredStones = useMemo(() => {
    return stones.filter(s => {
      return s.code.toLowerCase().includes(searchQuery.toLowerCase());
    });
  }, [stones, searchQuery]);
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-3">
           ...
           <div>
              ...
              <h2 className="text-2xl font-bold text-stone-900">Ru-Bkk 16042025</h2>
           </div>
        </div>
        {!isReadOnly && (
