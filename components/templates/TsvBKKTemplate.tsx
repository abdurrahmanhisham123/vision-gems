...
const generateMockData = (): BatchStone[] => {
  return Array.from({length: 40}).map((_, i) => ({
    id: `bkk-tsv-${i}`,
    code: `K-TS-${String(i+1).padStart(3, '0')}`,
    weight: Number((0.8 + Math.random() * 2.5).toFixed(2)),
    variety: 'Tsavorite',
          ...
          <div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-sm space-y-4">
               <div className="flex justify-between items-center py-2 border-b border-stone-50 last:border-0">
                  ...
                  <span className="text-sm font-bold text-stone-800">TSV BKK</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-stone-50 last:border-0">
                  <span className="text-sm text-stone-500">Last Audited</span>
                  <span className="text-sm font-bold text-stone-800">Dec 2024</span>
               </div>
            </div>
          </div>
        </div>
...
export const TsvBKKTemplate: React.FC<Props> = ({ tabId, isReadOnly }) => {
    ...
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto min-h-screen bg-stone-50/50">
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-3">
           <div className="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-200"><Layers size={24} /></div>
           <div>
        ...
        {!isReadOnly && (
           <button className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg hover:bg-emerald-700 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
         ...
         <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-sm flex items-center justify-between">
            <div>
               ...
               <div className="text-2xl font-bold text-emerald-600">{totalWeight.toFixed(2)} <span className="text-sm text-stone-400 font-medium">ct</span></div>
            </div>
            <Layers size={24} className="text-emerald-200" />
         </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm mb-8 flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
           ...
           <input 
             ...
             className="w-full pl-11 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
           />
        </div>
            ...
            <table className="w-full text-left border-collapse">
               ...
               <tbody className="divide-y divide-stone-100 text-[14px]">
                  {filteredStones.map(stone => (
                     <tr key={stone.id} className="hover:bg-emerald-50/20 transition-colors cursor-pointer group" onClick={() => setSelectedStone(stone)}>
                        ...
                        <td className="p-6 text-center">
                           <ChevronRight size={18} className="text-stone-300 group-hover:text-emerald-600 transition-colors" />
                        </td>
                     </tr>
                  ))}
               </tbody>
