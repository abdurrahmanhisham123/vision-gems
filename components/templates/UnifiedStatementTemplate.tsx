...
export const UnifiedStatementTemplate: React.FC<Props> = ({ moduleId, tabId, isReadOnly = false }) => {
  ...
  const handlePrint = () => {
    ...
    const tableRows = filteredItems.map(item => {
      ...
      const person = (item.person || '-').toString().replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      <tr>
        ...
        <td style="border: 1px solid #cccccc; padding: 5px 4px; font-size: 8pt; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; vertical-align: middle;">${person}</td>
        ...
        <table style="width: 100%; table-layout: fixed; border-collapse: collapse; margin-top: 10px; font-size: 8pt;">
          ...
          <thead>
            <tr>
              ...
              <th style="background-color: #f0f0f0; border: 1px solid #000000; padding: 6px 4px; text-align: left; font-weight: bold; font-size: 7pt; text-transform: uppercase; white-space: nowrap; color: #000000; vertical-align: middle;">Person</th>
          ...
          <tbody>
            ${tableRows || '<tr><td colspan="12" style="text-align: center; padding: 20px; border: 1px solid #cccccc;">No transactions found</td></tr>'}
          </tbody>
        </table>
      </div>
    `;
    ...
    <div className="p-4 md:p-8 max-w-[1920px] mx-auto min-h-screen bg-stone-50/20 pb-32 md:pb-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 md:gap-6 mb-6 md:mb-8">
        <div className="w-full lg:w-auto">
           ...
           <p className="text-stone-400 text-xs md:text-sm mt-1 font-medium">Unified Statement Report in use</p>
        </div>
